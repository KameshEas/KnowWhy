...
Please provide the brief now.`;
    ...
    for (let attempt = 0; attempt < 2; attempt++) {
      if (parsed) {
        ...
        if (valid) {
          ...
          // Fast path success
          metrics.increment('decision_brief_generation_success_total');
          ...
          // Count this repair attempt
          metrics.increment('decision_brief_generation_repair_attempts_total');
    ...
    metrics.increment('decision_brief_generation_failure_total');
    return {
      brief: parsed,
      valid: false,
